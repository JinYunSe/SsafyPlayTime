...
namespace StarterAssets
{
    ...
    public partial class StarterAssetsDeployMenu : ScriptableObject
    {
        ...
        {
            Type cinemachineVirtualCameraType = ResolveType("Cinemachine.CinemachineVirtualCamera");
            if (cinemachineVirtualCameraType == null)
            {
                Debug.LogWarning("Cinemachine is not available. Skipping virtual camera setup.");
                return;
            }
            ...
            if (!vcam)
            {
                if (TryLocatePrefab(CinemachineVirtualCameraName, new string[]{prefabFolder}, new[] { cinemachineVirtualCameraType }, out GameObject vcamPrefab, out string _))
                {
        ...
        {
            Type cinemachineBrainType = ResolveType("Cinemachine.CinemachineBrain");
            if (cinemachineBrainType == null)
            {
                Debug.LogWarning("Cinemachine is not available. Skipping main camera setup.");
                return;
            }
            ...
            if (mainCameras.Length < 1)
            {
                ...
                if (TryLocatePrefab(MainCameraPrefabName, new string[]{inFolder}, new[] { cinemachineBrainType, typeof(Camera) }, out GameObject camera, out string _))
                {
            ...
            else
            {
                ...
                if (!mainCameras[0].TryGetComponent(cinemachineBrainType, out _))
                {
                    mainCameras[0].AddComponent(cinemachineBrainType);
                }
            }
        }
            ...
            GameObject cinemachineVirtualCamera)
        {
            ...
            var vcamComponent = cinemachineVirtualCamera.GetComponent(cinemachineVirtualCameraType);
            if (vcamComponent == null)
            {
                return;
            }

            var serializedObject = new SerializedObject(vcamComponent);
